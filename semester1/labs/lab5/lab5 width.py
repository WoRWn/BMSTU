...
# Назначение программы: вывести таблицу значений функции на отрезке [-1;1] с шагом 0.1 и построить ее график, с динамичной шириной графика
...
width = int(input('Введите ширину построения графика (не менее 40 символов): '))
step_of_notch = (2 - (2 / width)) / (notches - 1)  # шаг вывода засечек
...
for i in range(notches - 1):
    ...
    axis_y += (' ' * (width // 2) + '{:5.2g}'.format(notch))[((-width + 7)//(notches-1)):]
    # добавление засечки в ось
print(axis_y)  # вывод оси Оу

# Блок 5: построение графика функции
r = r_min  # переменная для вычисления значений
for _ in range(iterations):
    f = r ** 2 - cos(pi * r) ** 2  # вычисление значения функции
    pos = int(f / (2 / width))  # вычисление позиции точки графика с заданной шириной
    if abs(r) < eps:  # проверка точности
        r = 0
    if abs(f) < eps:  # проверка точности
        f = 0
    if -(width // 2) <= pos < 0:
        print('{0:9.5g}|'.format(r) + ((width//2) + pos)*' ' + '*' + abs(pos + 1)*' ' + '|' + ((width//2) - 1)*' ')
        # положительная по оси Ox точка в требуемом диапазоне
    elif 0 < pos <= ((width // 2) - 1):
        print('{0:9.5g}|'.format(r) + (width // 2)*' ' + '|' + (pos - 1)*' ' + '*' + (((width // 2) - 1) - pos)*' ')
        # отрицательная по оси Ox точка в требуемом диапазоне
    elif pos == 0:
        print('{0:9.5g}|'.format(r) + (width // 2) * ' ' + '*' + ((width // 2) - 1) * ' ')  # точка в 0 по оси Ох
    else:
        print('{0:9.5g}|'.format(r) + (width // 2) * ' ' + '|' + ((width // 2) - 1) * ' ')  # точка вне требуемого
        # диапазона
    r += step

# Блок 6: вывод количества значений функции f1 на промежутке [-0.5;0.5]
print('Количество значений функции f1 на промежутке [-0.5;0.5] равно: {}'.format(amount))
# вывод количества значений функции на диапазоне
